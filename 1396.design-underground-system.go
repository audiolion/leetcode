...
func (ug *UndergroundSystem) CheckIn(id int, stationName string, t int) {
	ug.checkins[id] = checkin{stationName: stationName, time: t}
}

func (ug *UndergroundSystem) CheckOut(id int, stationName string, t int) {
	checkin := ug.checkins[id]
	times, exists := ug.travelTimes[stationName]
	if !exists {
		ug.travelTimes[stationName] = map[string][]int{
			checkin.stationName: {t - checkin.time},
		}
	} else {
		times[checkin.stationName] = append(times[checkin.stationName], t-checkin.time)
	}
	delete(ug.checkins, id)
}

func (ug *UndergroundSystem) GetAverageTime(startStation string, endStation string) float64 {
	ts, ok := ug.travelTimes[endStation][startStation]
	if !ok {
		return 0
	}
